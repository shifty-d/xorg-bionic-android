...
int mzOpenZipArchive(const char* path, ZipArchive* pArchive)
{
    int err;

    MemMapping* map = malloc (sizeof(MemMapping));
    pArchive->mapping = map;
    ...
    pArchive->addr = map->addr;
    pArchive->length = map->length;
...
void mzCloseZipArchive(ZipArchive* pArchive)
{
    ...
    sysReleaseMap(pArchive->mapping);
