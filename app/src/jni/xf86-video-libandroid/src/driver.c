...
#define HWC_NAME "libandroid"
#define HWC_DRIVER_NAME "libandroid"
...
_X_EXPORT DriverRec libandroid = {
    HWC_VERSION,
    HWC_DRIVER_NAME,
    Identify,
    Probe,
...
static XF86ModuleVersionInfo libandroidVersRec =
{
	"libandroid",
	MODULEVENDORSTRING,
	MODINFOSTRING1,
 ...
 */
_X_EXPORT XF86ModuleData libandroidModuleData = { &libandroidVersRec, Setup, NULL };
...
Setup(pointer module, pointer opts, int *errmaj, int *errmin)
{
    ...
    if (!setupDone) {
        setupDone = TRUE;
        xf86AddDriver(&libandroid, module, HaveDriverFuncs);
...
Identify(int flags)
{
    xf86PrintChipsets(HWC_NAME, "Driver for Android devices", Chipsets);
}
...
PreInit(ScrnInfoPtr pScrn, int flags)
{
    ...
    if (!NAInit(pScrn)) {
        xf86DrvMsg(pScrn->scrnIndex, X_ERROR,
                    "failed to initialize NativeActivity API\n");
        return FALSE;
    }
...
*/
	hwc->renderer.pixels = malloc(sizeof(uint32_t) * hwc->hwcWidth * hwc->hwcHeight);
